...
def main(file_path, product_code):
    ...
    reg.fit(combined_val_forecast, validation.values)
    ...
    reg_combined_forecast = reg.predict(combined_forecast)
    ...
    if sarima_forecast is not None and arima_forecast is not None and neural_network_forecast is not None:
        ...
        # Combine SARIMA and Neural Network forecasts using the regression model
        plt.plot(forecast_index, reg_combined_forecast, label='Combined Forecast', linestyle='--')
