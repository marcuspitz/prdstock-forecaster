...
def main(file_path, product_code):
    ...
    combined_val_forecast = pd.DataFrame({
    ...
    if combined_val_forecast.isnull().values.any():
        combined_val_forecast = combined_val_forecast.fillna(combined_val_forecast.mean())

    # Train stacking model on the validation set
    base_learners = [
        ('sarima', LinearRegression()),
        ('arima', LinearRegression()),
        ('nn', LinearRegression())
    ]
    stacker = StackingRegressor(estimators=base_learners, final_estimator=LinearRegression())
    stacker.fit(combined_val_forecast, validation.values)
    ...
    # Handle NaNs in the combined forecasts
    if combined_forecast.isnull().values.any():
        combined_forecast = combined_forecast.fillna(combined_forecast.mean())

    # Predict using the stacking model
    stacker_combined_forecast = stacker.predict(combined_forecast)
    ...
    if sarima_forecast is not None and arima_forecast is not None and neural_network_forecast is not None:
        ...
        # Combine SARIMA and Neural Network forecasts using the stacking model
        plt.plot(forecast_index, stacker_combined_forecast, label='Stacked Forecast', linestyle='--')
